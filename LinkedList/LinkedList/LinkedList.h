...
namespace ds
{
	namespace stack
	{
		/* A stack class that stores an array in stack order */
		template <class T>
		class stack_array
		{
		private:
		...
		public:
			stack_array(unsigned size) : _stack_size(size), _data_size(0), _array(nullptr) { _array = (T*)malloc(sizeof(T) * size); }
			stack_array(unsigned size, T* data)
			{
				_data_size = 0;
				_stack_size = size;
				_array = data;
			...
			size_t& size() { return _data_size; }

			inline void push(T data)
			{
				++_data_size;
				_array[_data_size - 1] = data;
			}

			inline void pop()
			{
				_array[_data_size - 1] = T();
				--_data_size;
			}
